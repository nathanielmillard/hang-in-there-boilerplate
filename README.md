...
### Future Iterations - conDevelop practical features that allow a better UI/UX for our website
   * Complete the full scope of the project (iteration 4) which involves creating a double click event listener on a saved posters page allowing the user more control to remove saved posters.
   * Continue developing customization features for our saved posters page that would allow the user to drag and reorder posters, adding a toggle event listener that would allow the user to expand each image based on a mouse click, then resize back to original on second mouse click 
   * Create an error message if the user accidentally double clicks on the "Show my poster" button
